...
/**
 * Get TikTok video download URL. Tries Lovetik first, then SSSTik.
 * @param {string} link - Full TikTok URL (short links should be expanded first)
 * @returns {Promise<{url:string, title:string, author:string, cover?:string, quality:string}|null>}
 */
async function getTikTokVideoUrl(link) {
  let result = await fetchViaLovetik(link);
  if (result) return result;
  result = await fetchViaSSSTik(link);
  return result;
}
